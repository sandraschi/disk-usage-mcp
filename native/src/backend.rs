...
const BACKEND_NAME: &str = "disk-usage-mcp-backend.exe";
const BACKEND_PORT: u16 = 11114;
const BACKEND_TAG: &str = "disk-usage-mcp-backend-x86_64-pc-windows-msvc.exe";
const ENV_PORT: &str = "MCP_PORT";
const ENV_HOST: &str = "MCP_HOST";
const ENV_TAURI: &str = "DISK_USAGE_TAURI";
...
fn free_port(port: u16) -> bool {
    #[cfg(windows)]
    {
        let img_kill = "Stop-Process -Name 'disk-usage-mcp-backend' -Force -ErrorAction SilentlyContinue; \
             Stop-Process -Name 'disk-usage-mcp-native' -Force -ErrorAction SilentlyContinue; \
             taskkill /F /IM disk-usage-mcp-backend.exe /T 2>$null; \
             taskkill /F /IM disk-usage-mcp-native.exe /T 2>$null";
            ...
            if i == 15 {
                let elevated = format!(
                    "Start-Process powershell -Verb RunAs -WindowStyle Hidden -ArgumentList \
                     '-NoProfile -Command \"Stop-Process -Name disk-usage-mcp-backend -Force -ErrorAction SilentlyContinue; \
                     taskkill /F /IM disk-usage-mcp-backend.exe /T 2>$null; \
                     Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue | \
                     ForEach-Object {{ taskkill /F /PID $_.OwningProcess /T 2>$null }}\"'"
                );
